...
async function sendMailForgetPassword(userEmail) {
  ...
  let resetToken = jwt.sign(payload, process.env.SECRETEKEY, {
    expiresIn: "1d",
  });
  ...
  try {
    ...
    console.log(await registerModel.findOne({ email: userEmail }));
    const dbResponse = await registerModel.updateOne(
      { email: userEmail },
      {
        $set: {
          resetPasswordToken: resetToken,
          resetPasswordTokenExpire: false,
        },
      }
    );
